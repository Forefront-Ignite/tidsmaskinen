...
- **Calls tab scope**: `TeamsCallsView` shows only mic-active sessions that do *not* overlap any calendar event. Booked time is canonical in Meetings; Calls is the ad-hoc surface (Slack huddles, impromptu FaceTime, etc.). Overlap is checked at reload time; nothing is written to `MicSession`.
