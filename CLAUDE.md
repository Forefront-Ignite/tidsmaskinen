...
- **Calls tab scope**: `TeamsCallsView` shows mic-active intervals minus any time covered by a calendar event. A mic session that runs from 13:50 to 15:20 against a booked 14–15 meeting renders as two `CallSegment` rows (13:50–14:00 and 15:00–15:20), so meeting over/undershoot still surfaces here while the booked hour stays canonical in Meetings. Sessions fully covered by events are hidden and surfaced as a count in the empty state. Segments under 30s are dropped to filter mic flicker at meeting boundaries. Attribution writes to the underlying `MicSession.id`, so all segments of the same session share a customer/project — by design (one mic session = one ad-hoc context).
