...
- **Review evidence and previews**: `ReviewQueue.rows` also returns `ReviewRow.evidence` — the three longest stretches of consecutive samples on a signal (a gap over `evidenceGapSeconds` = 2 min starts a new one; open rows list only open stretches), each with the window title (URL path for a host) seen most, which the detail pane links into My day, plus a remainder line for the shorter stretches so the card sums to the header. Review's decision note names an existing rule for the same pattern by another customer (`existingRuleNote`) and, for Always, what the rule also clears in the other weeks of the backlog window (`loadImpact`, resolved off the main thread per selection). Assigning a whole host warns when the host is in `sharedHosts` (localhost, Microsoft/Azure portals, GitHub, Atlassian) or already ruled to a customer — the paths are the right unit there. The Customers rule editor previews what a pattern matches in the last 90 days via `AppDatabase.ruleMatchCount` (grouped by signal column, then `RuleMatcher.matches` per distinct value; calls counted from mic sessions).
